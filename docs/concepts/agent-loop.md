...
- **`before_turn_developer_instructions`**: runs after OpenClaw builds the turn-level developer instructions string and before native Codex `turn/start`, allowing plugins to replace or prepend/append that string without changing thread-start instructions.
